function [h] = plot_sensor_ARCore_frame(R_gc, p_gc, camScale, camBodyColor)
% Project:    Astrobee's mapping and localization system
% Function:  plot_sensor_ARCore_frame
%
% Description:
...
camBody = zeros(3,4);
camBody(:,1) = camScale * [(cols/2);-(rows/2);-(cols-50)] / 1000;
camBody(:,2) = camScale * [-(cols/2);-(rows/2);-(cols-50)] / 1000;
camBody(:,3) = camScale * [-(cols/2);(rows/2);-(cols-50)] / 1000;
camBody(:,4) = camScale * [(cols/2);(rows/2);-(cols-50)] / 1000;
